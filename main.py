...
app = FastAPI(
    ...
    description="Einfacher API-Endpoint für Excel zu CSV Konvertierung mit E-Mail-Versand",
    version="1.0.0"
)
...
def convert_excel_to_csv(input_file_path: str, mandant: str, abrechnungsmonat: Optional[str], sheet_name: str):
...
def send_email(recipient_email: str, file_path: str, abrechnungsmonat: str, rows_count: int) -> bool:
    """Sendet die CSV-Datei per E-Mail"""
    try:
...
async def convert_payroll(
    ...
    if not file.filename.endswith(('.xlsx', '.xls')):
        raise HTTPException(status_code=400, detail="Nur Excel-Dateien erlaubt")
